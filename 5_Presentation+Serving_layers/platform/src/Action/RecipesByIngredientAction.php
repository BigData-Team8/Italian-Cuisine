...
final class RecipesByIngredientAction {
    private $twig;
    ...
    public function __invoke(ServerRequestInterface $request, ResponseInterface $response, $args): ResponseInterface {
        ...
        if ($request->getMethod() == 'GET') {
            ...
            $params = [
                'query' => [
                    'ingredient' => $args['ingredient'] ? $args['ingredient'] : $request->getQueryParams()['ingredient'],
                    'limit' => $request->getQueryParams()['limit']
                ]
            ];

            $baseURL = $this->configuration->getParam('APIbaseURL');
            $API_response = $client->request('GET', $baseURL . '/api/recipes/by-ingredient', $params);
